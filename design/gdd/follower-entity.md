...
> **Status**: In Review (major revision 2026-04-22; 2026-04-24 consistency-check sync — `collision_transfer_per_tick = 2` → dynamic `∈ [1, 4]`; 2026-04-24 Batch 3 — Overview tier-2 render reference "4 on a distant one" → "1 billboard impostor per crowd" per Follower LOD Manager sole-owner declaration; 2026-05-02 Overview rig prose synced to §C.1 canonical "2-Part Body+Hat composite" per ADR-0007 acceptance + registry `humanoid_on_followers`)
...
**Implementation note** (gameplay-programmer): `FollowerEntityClient` lives at `src/ReplicatedStorage/Source/FollowerEntity/Client.luau` as a per-crowd class (per ADR-0006 §Source Tree Map + ADR-0007 §Key Interfaces). Per-follower state stored as parallel arrays (not per-instance table keys) for Luau table-iter cache locality. Coordinator singleton `CrowdManagerClient` lives at `src/ReplicatedStorage/Source/FollowerEntity/CrowdManagerClient.luau`, owns `{[string]: FollowerEntityClient.ClassType}` map + per-crowd `Janitor`, and drives all updates from one `RenderStepped` connection.
